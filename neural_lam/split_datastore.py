...
def main():
    ...
    parser = argparse.ArgumentParser(description="Split a datastore into interior and boundary datastores.")
    parser.add_argument(
        "--source_config_path",
        type=str,
        required=True,
    ...
    start_time_str = str(output_splitting.splits['train'].start)  # Example start time as string
    ...
    for split in cfg.output.splitting.splits:
        ...
        cfg.output.splitting.splits[split].end = (start_time + time_interval).isoformat()
    ...
    logger.info(f"Calculating convex hull with margin width of {max_dist} degrees.")
    ds_boundary, mask = mdp.ops.cropping.crop_with_convex_hull(
        ds=datastore, ds_reference=datastore, margin_thickness=max_dist, include_interior_points=True, return_mask=True, #invert=True
    )
    ...
    for ipt in cfg_boundary.inputs.values():
        ipt.coord_ranges['cell'] = list(set(boundary_ind).union(set(interior_ind)))
        if ipt.target_output_variable == 'state':
            ipt.target_output_variable = 'forcing'
            ipt.dim_mapping['forcing_feature'] = ipt.dim_mapping['state_feature']
    ...
    cfg_boundary.output.variables["forcing"] = ["time", "grid_index", "forcing_feature"]

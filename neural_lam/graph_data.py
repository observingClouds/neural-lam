...
from .utils import zero_index_edge_index
...
class GraphEdgesAndFeatures:
    ...
    mesh_lat_lon: Tuple[Tensor, ...] = field(default_factory=tuple)
...
def load_graph(
    ...
    m2m_edge_index_list = [
        reindex_func(tensor).clone() for tensor in loads_file("m2m_edge_index.pt")
    ]
    ...
    m2m_features_list = [
        reindex_func(tensor).clone() for tensor in loads_file("m2m_features.pt")
    ]
    g2m_features = loads_file("g2m_features.pt")
    m2g_features = loads_file("m2g_features.pt")

    mesh_static_features_list = loads_file("m2m_node_features.pt")

    # Determine 2d or 3d features
    node_feature_dims = mesh_static_features_list[0].shape[-1]
    ...
    if three_dim_features:
        ...
        mesh_lat_lon = [
            ...
            for mesh_coords in mesh_static_features_list
        ]
    ...
    if hierarchical:
        ...
        mesh_static_features_list = mesh_static_features_list[0]
